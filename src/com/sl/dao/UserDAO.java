...
public class UserDAO {
	...
	public static boolean verifyEmailHash(String user_id, String hash) throws DBException {
		...
		try {
			...
			DBConn.close(conn, ps, res);
			LOGGER.debug(e.getMessage());
			throw new DBException("Exception while accessing database ffff");
		}
		return verified;
	}
	...
	public static UserPojo verifyLogin(String inputEmail, String inputPassword) throws DBException {
		...
		try {
			conn = DBConn.getConnection();
			ps = conn.prepareStatement("select user_id, email, first_name, last_name, status, created_time from demo_user where email = ? and pasword = ?");
